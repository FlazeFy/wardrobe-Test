...
describe('Wardrobe E2E Test - TC-EX-005 - Export', () => {
    const username = 'flazefy'
    const password = 'nopass123'
    const date = new Date().toISOString().replace(/:/g, '-')

    it('User Can Download Exported Data Of Apps History', () => {
        ...
        cy.templateE2ELogin(username, password).then(() => {
            ...
            // Step 2: At the "Export My Data" section, find the sub section "Apps History Data". Click button "Download Excel" to export clothes data
            cy.get('#export_data-section').should('exist').within(()=>{
                cy.get('h2').contains('Export My Data')
                cy.get('#export_data_apps_history-section').should('exist').within(()=>{
                    cy.get('h5').contains('Apps History Data')
            ...
            // Evidence - Step 3
            cy.screenshot(`TC-EX-005_Step-3-${date}`)
        })
    })
})
