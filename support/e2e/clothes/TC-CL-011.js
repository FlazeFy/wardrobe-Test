...
describe('Wardrobe E2E Test - TC-CL-011 - Clothes', () => {
    ...
    it('User Cant Add A Clothes With Duplicated Clothes Name', () => {
        ...
        cy.templateE2ELogin(username, password).then(() => {
            ...
            // Step 2: After section with title "All Clothes", there will be list of clothes that contain the clothes name. Use the name to create a new clothes
            cy.get('#clothes_holder .box-clothes').first().within($el => {
                // Clothes Name
                cy.wrap($el).find('h4').should('exist').and('not.be.empty').invoke('text').then(text => {
                    clothes_data.clothes_name = text
                })
            })
            // Evidence - Step 2
            cy.screenshot(`TC-CL-011_Step-2-${date}`)

            // Step 3: Search "Add Clothes" button
            cy.contains('a', 'Add Clothes').click()
            cy.url().should('include', '/clothes/add')

            // Step 4: Fill the Clothes Data in Form at the Section "Add Form" 
            cy.contains('.form-container', 'Add Form').within(() => {
                ...
                // Evidence - Step 4
                cy.screenshot(`TC-CL-011_Step-4-${date}`)

                // Step 5: Click the "Save Changes" button
                cy.contains('button', 'Save Changes').click()
            });

            // Step 6: A Failed Pop Up will appear with text "clothes name has been used. try another". Click "Okay!"
            cy.get('.swal2-popup:not(.swal2-loading)', { timeout: 10000 }).should('exist').then(() => {
                cy.get('.swal2-html-container').invoke('text').then((text)=>{
                    expect(text).to.equal(`clothes name has been used. try another`)
                })
                // Evidence - Step 6
                cy.screenshot(`TC-CL-011_Step-6-${date}`)
